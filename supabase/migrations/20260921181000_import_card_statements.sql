-- Stage 5: one atomic entry point for explicit card-statement adapters.
-- Parsed rows arrive from server-side adapters; the original file and the raw
-- payload remain evidence, while transactions and future installments remain
-- editable interpretations.
create function public.import_card_statement(
  p_sha256 text,
  p_filename text,
  p_mime_type text,
  p_size_bytes bigint,
...
begin
  ...
  -- A file can contain more than one physical card (notably Caixa). The
  -- import itself deliberately stays card-neutral; statements and rows carry
  ...
  loop
    ...
    if item_kind = 'purchase' and item ? 'installment' and item->'installment' is not null then
